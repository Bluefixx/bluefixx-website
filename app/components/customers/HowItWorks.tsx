    ...
    <section className="w-full py-[64px] md:py-[96px] px-4 md:px-6 lg:px-[64px] bg-[#001B40]">
      <div className="max-w-6xl mx-auto flex flex-col items-center">
        ...
        <div className="relative w-full flex flex-col md:flex-row items-center md:items-start justify-between gap-12 md:gap-4">
          {/* Connector Line */}
          <div className="absolute top-[48px] md:top-[60px] left-1/2 md:left-0 w-[2px] md:w-full h-full md:h-[2px] bg-[#CFC4C5] -translate-x-1/2 md:translate-x-0 z-0" />

          {steps.map((step) => (
            <div key={step.id} className="relative z-10 flex flex-col items-center text-center flex-1 group">
              {/* Step Number Circle */}
              <div 
                className="w-[96px] h-[96px] md:w-[120px] md:h-[120px] rounded-full flex items-center justify-center mb-6 shadow-lg transition-transform group-hover:scale-105"
                style={{
                  background: "radial-gradient(circle, #00D2FF 0%, #001B40 100%)", // Desktop default
              ...
              >
                ...
                <style jsx>{`
                  div {
                    --circle-bg: radial-gradient(circle, #00D2FF 0%, #001B40 100%);
                  }
                  @media (max-width: 768px) {
                    div {
                      --circle-bg: radial-gradient(circle, #001128 0%, #0092D0 100%);
                    }
                  }
                `}</style>
