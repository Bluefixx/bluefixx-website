...
export default function ReusableTestimonials({
  testimonials,
  heading,
  subheading,
  mobileBgColor = "bg-[#F7F9FB]",
  desktopBgColor = "bg-white",
  headingStyles,
  subheadingStyles,
}: TestimonialsProps) {
  ...
  return (
    <section className={`w-full ${desktopBgColor} py-[64px] md:py-[96px] px-6 lg:px-[64px] relative`}>
      <div className={`md:hidden absolute inset-0 ${mobileBgColor} -z-10`} />
      <div className="max-w-6xl mx-auto">
        {/* Header Section */}
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 md:mb-16">
          <div className="flex flex-col gap-3 md:gap-4 max-w-2xl">
            <h2 className={headingStyles || "font-poppins font-semibold md:font-bold text-[24px] md:text-[30px] leading-[28px] md:leading-[36px] tracking-[0px] text-[#191C1E] md:text-black"}>
              {heading}
            </h2>
            <p className={subheadingStyles || "font-montserrat font-normal text-[16px] leading-[24px] tracking-[0px] text-[#45464D] md:text-[#4C4546]"}>
              {subheading}
            </p>
          </div>
...
function TestimonialCard({ testimonial, isMobile }: { testimonial: TestimonialData, isMobile: boolean }) {
  return (
